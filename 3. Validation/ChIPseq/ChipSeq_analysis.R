...
rep1.200bins = BinChIPseq(MeCP2_rep1, bins)
...
bm = BiomartGeneRegionTrack(genome="mm9", 
                            biomart=mart,filter=list("with_refseq_dna"=TRUE), 
                            size=4, name="RefSeq", utr5="red3", utr3="red3", 
                            protein_coding="black", col.line=NULL, cex=7,
                            collapseTranscripts="longest",
                            featureMap=fm)

test <- listFilters(mart)

AT = GenomeAxisTrack( )

# test to see if it works
MeCP2_rep1.track = DataTrack(MeCP2_rep1_fil,
                       strand="*", genome="mm9", col.histogram='steelblue',
                       fill.histogram='steelblue', name="MeCP2\n(Rep. 1)", col.axis='black',
                       cex.axis=0.4, ylim=c(0,50))

MeCP2_rep2.track = DataTrack(MeCP2_rep2_fil,
                       strand="*", genome="mm9", col.histogram='steelblue',
                       fill.histogram='steelblue', name="MeCP2\n(Rep. 2)", col.axis='black',
                       cex.axis=0.4, ylim=c(0,50))

Input_rep1.track = DataTrack(Input_rep1_fil,
                             strand="*", genome="mm9", col.histogram='lightgrey',
                             fill.histogram='lightgrey', name="Input\n(Rep. 1)", col.axis='black',
                             cex.axis=0.4, ylim=c(0,50))

Input_rep2.track = DataTrack(Input_rep2_fil,
                             strand="*", genome="mm9", col.histogram='lightgrey',
                             fill.histogram='lightgrey', name="Input\n(Rep. 2)", col.axis='black',
                             cex.axis=0.4, ylim=c(0,50))


# The Dlk1-Dio3 imprinted region (Chromosome 12)
plotTracks(c(MeCP2_rep1.track, MeCP2_rep2.track, Input_rep1.track, Input_rep2.track, bm, AT),
           from=110630000, to=111590000, chromosome = "chr12",
           transcriptAnnotation="symbol", window="auto", 
           type="histogram", cex.title=0.7, fontsize=10 )

# The entire chromosme
plotTracks(c(MeCP2_rep1.track, MeCP2_rep2.track, Input_rep1.track, Input_rep2.track,AT),
           from=1, to=121257530, chromosome = "chr12",
           transcriptAnnotation="symbol", window="auto", 
           type="histogram", cex.title=0.7, fontsize=10 )
...
# Imprinted genes in data
imprintDF <- read_xlsx("D:/RTTproject/CellAnalysis/CHIPSeq/imprinted_genes_mouse.xlsx")
...
# Get counts around TSS of imprinted genes
...
# Get counts around TSS of non-imprinted genes
...
ggplot() +
  geom_line(data = plotDF, aes(x = location, y = value, group = group, color = group),
            linewidth = 1.5) +
  ylab("Mean tag count") +
  xlab("Distrance from the TSS") +
  labs(color = NULL) +
  theme_bw() +
...
save(plotDF, plotDF_permutation, 
     file = "D:/RTTproject/CellAnalysis/CHIPSeq/plotDF_ChIPSeq.RData")
...
ggsave(p, 
       file = "D:/RTTproject/CellAnalysis/CHIPSeq/ChipSeqPlot.png",
       width = 6, height = 4.5)
