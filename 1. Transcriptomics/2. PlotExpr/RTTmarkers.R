...
pvalues <- matrix(unlist(lapply(topList, function(x){x[genes_all,"FDR"]})), ncol = length(topList))
...
test <- sum(rowSums(pvalues<0.05 & abs(logFCs) > 1)>3)
test/nrow(gxMatrix_norm)
...
#==============================================================================#
# select genes:
test <- geneAnnotation[which(str_detect(geneAnnotation$GeneName, "BDNF")),]
selectedGene <- "ENSG00000176697_BDNF" 

sum(pvalues[selectedGene,]<0.05 & abs(logFCs[selectedGene,]) > 1)
#==============================================================================#
...
# Save plot
ggsave(finalPlot, file = paste0("RTTmarkers/", nameGene,".png"), width = 6, height = 4)
